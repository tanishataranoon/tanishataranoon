<p align="center">
  <img src="banner.svg" width="100%" alt="Tanisha Taranoon Hridy banner"/>
</p>

<p align="center">
  <br/>
  <br/>
</p>


<!-- ========================================================= -->
<!--                       INTRO                               -->
<!-- ========================================================= -->

<h1 align="center">
  Hey, I'm Tanisha 👋
</h1>

<p align="center">
  <b>AI/ML • CSE • Research • Create</b>
</p>

<p align="center">
  <i>Somewhere between logic and imagination.</i>
</p>

<p align="center">
  <img
    src="https://readme-typing-svg.demolab.com?font=Georgia&size=18&duration=3000&pause=1200&color=A78BFA&center=true&vCenter=true&width=600&lines=Welcome+to+my+little+corner+of+the+internet;Exploring+AI+%26+Machine+Learning;Building%2C+breaking%2C+learning%2C+repeating;Between+logic+and+imagination"
    alt="Typing animation"
  />
</p>

<p align="center">
  <a href="https://github.com/tanishataranoon">
    <img
      src="https://img.shields.io/badge/GitHub-tanishataranoon-181717?style=for-the-badge&logo=github&logoColor=white"
    />
  </a>
  <!-- Add LinkedIn later -->
  <!-- Add Portfolio later -->
</p>

<br/>


<!-- ========================================================= -->
<!--                    🧠 A LITTLE ABOUT ME                   -->
<!-- ========================================================= -->

## 🌙 About Me

Hi! I'm **Tanisha Taranoon Hridy**, a CSE undergraduate who enjoys
building things, experimenting with ideas, and learning how intelligent
systems work.
I'm currently exploring **AI/ML, deep learning, computer vision, NLP,
and explainable AI**.
Outside the technical world, I'm an introvert who enjoys having my own
little universe — anime, manga, manhwa, music, drawing, cooking, and
quiet time.

<p align="center">
  <i>Curious mind. Quiet soul. Slightly obsessed with fictional worlds.</i>
</p>

---
## 🎧 Currently Into

<p align="center">

🎬 **Anime** &nbsp;·&nbsp;
📖 **Manga / Manhwa** &nbsp;·&nbsp;
🎨 **Drawing** &nbsp;·&nbsp;
🍳 **Cooking** &nbsp;·&nbsp;
🌙 **Quiet Time**
<br/>
🎧 **BTS** &nbsp;·&nbsp;
**Taylor Swift** &nbsp;·&nbsp;
**The Weeknd** &nbsp;·&nbsp;
**Chase Atlantic**
</p>
## 💻 Things I'm Building

<table>
<tr>

<td width="50%" valign="top">

### 🌾 Multi-Hazard Forecasting

**Undergraduate Thesis · 🌱 Ongoing**

AI-based multi-hazard forecasting and early warning using hybrid
approaches and Explainable AI.

`AI` `Deep Learning` `Forecasting` `XAI`

</td>

<td width="50%" valign="top">

### 🎭 Multi-Face Deepfake Detection

**Research Project · 🧪 Ongoing**

Exploring deepfake detection in multi-person videos through face
analysis, tracking, and reliable prediction.

`Computer Vision` `Deep Learning` `Video`

</td>

</tr>

<tr>

<td width="50%" valign="top">

### 🌳 Rare Tree Locator

**✨ Built**

A web application for discovering and tracking rare trees.

`Django` `Python` `JavaScript`

</td>

<td width="50%" valign="top">

### 🧪 Toxic Comment Detection

**✨ Built**

An NLP project exploring machine learning and deep learning for
toxic comment classification.

`NLP` `ML` `Deep Learning`

</td>

</tr>

<tr>

<td width="50%" valign="top">

### 🤖 Gesture-Controlled Robot

**✨ Built**

A robotics project combining gesture control and embedded systems.

`Robotics` `Embedded Systems`

</td>

<td width="50%" valign="top">

### 💬 Sentiment Analysis

**✨ Built**

An NLP project focused on sentiment classification.

`NLP` `Machine Learning`

</td>

</tr>
</table>

<p align="center">
  <i>More projects coming as I build, learn, and experiment.</i>
</p>

---

## 🔬 What I'm Exploring

<p align="center">

<img src="https://img.shields.io/badge/AI-8B5CF6?style=flat-square"/>
<img src="https://img.shields.io/badge/Machine%20Learning-7C3AED?style=flat-square"/>
<img src="https://img.shields.io/badge/Deep%20Learning-6D28D9?style=flat-square"/>
<img src="https://img.shields.io/badge/Computer%20Vision-5B21B6?style=flat-square"/>
<img src="https://img.shields.io/badge/NLP-9333EA?style=flat-square"/>
<img src="https://img.shields.io/badge/Explainable%20AI-A855F7?style=flat-square"/>
<img src="https://img.shields.io/badge/Trustworthy%20AI-7E22CE?style=flat-square"/>

</p>

---

## 🛠️ Toolbox

**Languages**

`Python` `C` `Java` `JavaScript` `SQL`

**AI / ML**

`PyTorch` `TensorFlow` `Scikit-learn` `Hugging Face`

**Computer Vision**

`OpenCV` `YOLO`

**Development**

`Django` `HTML` `CSS`

**Tools**

`Git` `GitHub` `Linux` `Jupyter` `Google Colab` `VS Code` `Overleaf`

---

## 🎨 Outside the Code

<table align="center">
<tr>

<td align="center">
🎨<br/>
<b>Drawing</b>
</td>

<td align="center">
🍳<br/>
<b>Cooking</b>
</td>

<td align="center">
📖<br/>
<b>Anime / Manga</b>
</td>

<td align="center">
🎧<br/>
<b>Music</b>
</td>

<td align="center">
🌙<br/>
<b>Quiet Time</b>
</td>

</tr>
</table>

---

## 🏆 Little Wins

<p align="center">

🏅 **Vice Chancellor's Honor Award** · Spring 2024

&nbsp; · &nbsp;

🏅 **Vice Chancellor's Honor Award** · Spring 2025

&nbsp; · &nbsp;

🏅 **Vice Chancellor's Honor Award** · Fall 2025

<br/><br/>

🤖 **Inter University Robo Expo 3.0**

</p>

---

## 📊 GitHub Life

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=tanishataranoon&show_icons=true&theme=midnight-purple&hide_border=true&count_private=true" width="48%"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=tanishataranoon&layout=compact&theme=midnight-purple&hide_border=true" width="40%"/>
</p>

<p align="center">
  <img src="https://streak-stats.demolab.com/?user=tanishataranoon&theme=midnight-purple&hide_border=true" width="55%"/>
</p>

---

## 🌙 Say Hello

<p align="center">

<a href="https://github.com/tanishataranoon">
  <img src="https://img.shields.io/badge/GitHub-tanishataranoon-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

&nbsp;

<!-- Replace # with your LinkedIn URL -->
<a href="#">
  <img src="https://img.shields.io/badge/LinkedIn-Add%20Later-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

&nbsp;

<!-- Replace # with your portfolio URL -->
<a href="#">
  <img src="https://img.shields.io/badge/Portfolio-Add%20Later-7C3AED?style=for-the-badge&logo=googlechrome&logoColor=white"/>
</a>

</p>

<p align="center">
  <br/>
  🌙 ✦ ⋆ ✦ 🎧 ✦ ⋆ ✦ 🎨 ✦ ⋆ ✦ 🌙
  <br/><br/>
  <i>Thanks for visiting my little corner of the internet.</i>
  <br/><br/>
  <b>Somewhere between logic and imagination.</b>
  <br/><br/>
  🌙
</p>
